...
func TestQuickstartInstallScriptRendersValidBash(t *testing.T) {
	...
	t.Run("shellcheck", func(t *testing.T) {
		...
		if err != nil {
			// shellcheck ships in the dogfood test image, so its absence in CI
			// would silently drop this coverage. Require it there; allow local
			// runs without it.
			if testutil.InCI() {
				t.Fatal("shellcheck must be installed in CI so this coverage cannot silently vanish")
			}
...
func TestRenderBaseHonorsRegistryMirror(t *testing.T) {
	...
	// Every base must honor the mirror: once a mirror is configured, the public
	// default registry host must not survive into any module source, under any
	// namespace. Asserting NotContains(host+"/coder/") only closed the "coder/"
	// namespace, so a base hardcoding registry.coder.com/coder-labs/... (a
	// namespace that already ships in the catalog) passed green. Stripping
	// whole-line comments first drops the "# Module docs (public registry): ..."
	// links, which legitimately name the public registry, so only real HCL is
	// swept. Sorting BaseTemplateIDs() pins the otherwise map-ordered subtests.
	...
	for _, id := range ids {
		t.Run(id, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.NotContains(t, stripHCLCommentLines(string(rendered)),
				codersdk.DefaultTemplateBuilderRegistryURL,
				"base %q must render every module source against the configured registry, not the hardcoded default (any namespace)", id)
		})
	}
}
